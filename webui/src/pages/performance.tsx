  ...
  return (
    <div className="space-y-5">
      ...
      <div className="space-y-2 rounded-xl border border-blue-100 bg-blue-50/50 px-4 py-3 text-xs leading-relaxed text-slate-600">
        <p>{isZh ? "横轴按当前可见点的最低分向下、最高分向上取整至 10 的倍数自动缩放（满分 100），并在左右两端留出标记边距，极值点不会压在坐标轴上；纵轴为综合总体 TPS（总输出 Token / 上游总延时），包含首字延迟与 Prefill 开销，真实反映端到端吞吐，默认范围 0–100，超过 100 时自动扩大上限。每个变体取已保留日志中最近 50 次调用按有效样本合并。每个点显示该供应商的图标，虚线边框表示不足 3 个有效样本。不是主动测速或全历史平均。" : "X automatically rounds the visible minimum score down and maximum up to multiples of 10 (scores remain out of 100) and keeps marker room at both ends; Y is the overall TPS (total output tokens / total upstream latency), incorporating time-to-first-token and prefill latency, defaulting to 0–100 and expanding automatically above 100. Each variant averages its latest 50 retained calls. Every point shows its provider's icon; a dashed border marks fewer than 3 valid samples. This is not an active benchmark or an all-time average."}</p>
