  ...
  return (
    <div className="space-y-5">
      ...
      <div className="space-y-2 rounded-xl border border-blue-100 bg-blue-50/50 px-4 py-3 text-xs leading-relaxed text-slate-600">
        <p>{isZh ? "横轴按当前可见点的最低分向下、最高分向上取整至 10 的倍数自动缩放（满分 100），并在左右两端留出标记边距，极值点不会压在坐标轴上；纵轴为加权平均 TPS（输出 token/秒），默认范围 0–100，超过 100 时自动扩大上限。每个变体取已保留日志中最近 10 次调用求平均 TPS，再按有效样本数加权合并。每个点显示该供应商的图标，虚线边框表示不足 3 个有效样本。不是主动测速或全历史平均。" : "X automatically rounds the visible minimum score down and maximum up to multiples of 10 (scores remain out of 100) and keeps marker room at both ends, so an extreme point never sits on an axis; Y is the weighted average TPS (output tokens/second), defaulting to 0–100 and expanding automatically above 100. Each variant averages its latest 10 retained calls; variants merge weighted by valid samples. Every point shows its provider's icon; a dashed border marks fewer than 3 valid samples. This is not an active benchmark or an all-time average."}</p>
      ...
      {ready && <>
        ...
        <section className="glass min-w-0 rounded-2xl border border-slate-200">
          <div className="border-b border-slate-200/70 px-4 py-3"><h2 className="text-sm font-semibold text-slate-800">{isZh ? "能力 × 速度" : "Capability × speed"}</h2>
            <p className="mt-1 text-xs text-slate-500">{isZh ? "仅包络线上的点直接显示前缀与供应商名称（重合位置列出该处全部分组）；其余点悬停、聚焦或轻触查看。虚线连接当前可见分组的右上凸包边界；悬停、聚焦或轻触可查看评分、TPS、样本和包络线归属。" : "Only envelope points label prefixes and providers directly (coincident positions list every group at that spot); hover, focus or tap any other icon for its details. The dashed line follows the upper-right convex boundary of visible groups; hover, focus or tap for score, TPS, sample details and envelope membership."}</p></div>
        ...
        <details className="glass rounded-2xl p-4" open={missing.length > 0}>
          ...
          <div className="mt-3 overflow-x-auto"><table className="w-full min-w-[900px] text-left text-xs">
            <thead className="text-slate-500"><tr>{(isZh ? ["前缀 / 供应商", "评分 / TPS", "有效 / 已选请求", "样本时间", "状态 / 变体"] : ["Prefix / provider", "Score / TPS", "Valid / selected requests", "Sample times", "Status / variants"]).map((label) => <th key={label} className="p-2">{label}</th>)}</tr></thead>
            <tbody>{visible.map((row) => <tr key={row.key} className="border-t border-slate-200/70 align-top">
              ...
              <td className="p-2 tabular-nums">{row.score}/100<p>{row.status === "ready" ? formatTps(row.tps) : "–"}</p></td>
              ...
              <td className="max-w-80 p-2"><Badge variant={row.status === "ready" ? "success" : "outline"}>{row.status === "ready" ? (isZh ? "已绘图" : "Plotted") : (isZh ? "无有效 TPS" : "No valid TPS")}</Badge>
                ...
                {row.variants.length > 0 && <ul className="mt-1 space-y-0.5">{row.variants.map((variant) => (
                  <li key={variant.upstream_model} className="break-all font-mono text-[11px] text-slate-600">
                    {variant.upstream_model} · {variant.stats.average_tps !== null ? formatTps(variant.stats.average_tps) : "–"} · {variant.stats.valid_tps_count}/{variant.stats.selected_request_count}
                  </li>
                ))}</ul>}
              </td>
            </tr>)}</tbody>
