...
pub async fn get_model_performance(
    ...
    gw.admin()
        .get_model_performance(provider_id.as_deref())
        .await
        .map_err(|e| e.to_string())
}
...
#[tauri::command]
pub async fn clear_error_logs(gw: State<'_, Gateway>) -> Result<u64, String> {
    gw.admin()
        .clear_error_logs()
        .await
        .map_err(|e| e.to_string())
...
pub async fn get_provider_usage_detail(
    ...
    id: String,
    hours: Option<i32>,
) -> Result<ProviderUsageDetail, String> {
    gw.admin()
        .get_provider_usage_detail(&id, hours)
        .await
        .map_err(|e| e.to_string())
}
