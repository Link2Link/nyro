...
| `fallback` | 否 | 降级兜底行（默认 `false`）：不参与任何策略，仅追加在目标列表末尾——所有正常目标被跳过（配额/熔断/禁用）或可重试失败后才会调用。每个模型至多一个，且不能是唯一目标；建议放在不同 Provider（配额按 Provider 生效） |

### usage 用量优先策略

`usage` 复用后台 Provider 套餐用量监控，热路径只读取内存快照：

1. 每个供应商只按其**最大周期主窗口**（月 > 周 > 5 小时）计算"所需加速倍数" `r = 剩余额度% ÷ 剩余时间%`（剩余时间按该窗口 5h/7d/30d 周期从重置时间倒推）。
2. 全部可评分供应商在同一个池内按 `r³` 加权随机分流：`r > 1` 表示照当前节奏会在重置前留下未用额度（加大流量加速消耗），`r < 1` 表示会在重置前耗尽（让位）；重置临近而额度未烧完时 `r` 自动放大、占比持续攀升，重置后自动回落。`r` 上限 10。
3. 缺失重置时间时 `r` 退化为剩余额度比例；没有可识别主窗口的 Provider 排在最后，Provider 之间等权随机，同一 Provider 的多个 target 再按静态 `weight` 排列，避免重复 target 放大 Provider 份额。

任何窗口（包括未参与评分的较小窗口）达到 100% 后，仍会由配额调度器立即跳过。用量正常每 5 分钟刷新，429 会触发即时刷新；查询失败时继续沿用最后成功快照（含跨重置）。
