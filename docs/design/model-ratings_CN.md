...
综合分（评分本身仍为 0–100），下限按当前可见绘图点的最低分向下取整至 10 的倍数，上限按最高分向上取整至 10 的倍数，且始终限制在 0–100。单个点至少保留 10 分跨度；没有点时使用完整 0–100。例如 53–78 → 50–80，只有 73 → 70–80，只有 100 → 90–100。刻度间隔为 10，搜索和供应商筛选后重新计算坐标范围，但不改变实际评分或点的身份。
...
- 使用统一灰蓝色虚线，绘制在点和标签下方，不填充、不闭合、不延伸到坐标轴，不拦截鼠标。
  供应商颜色和空心/实心含义不变，详情及可访问描述明确标识边界模型。
