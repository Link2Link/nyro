...
服务端在最近 7 天内，为每个组合选取最近 10 次成功完整结束的请求，再平均有效的
逐请求 TPS。输出上限截断、失败、取消、未完整结束及不可信历史请求不计入；无效 TPS
不会用更早请求补位。流式 TPS 通常扣除首块等待；生成间隔小于 50ms，或首块等待
占上游总耗时至少 80% 时，改用全部上游耗时避免 TPS 尖峰。非流式使用观测到的上游耗时。有效样本
少于 3 个为空心点。缺少 TPS 不等于零；部分或整体统计失败明确展示，不回退到旧用量
平均值，也不会主动向上游测速。保留生命周期观测与历史元数据恢复；推理强度元数据
仅供内部诊断，不拆分界面统计。
