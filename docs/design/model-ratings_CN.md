# 供应商模型评分

[English](model-ratings.md)

## 目标

Nyro 为每个精确的 **`供应商 ID + 上游模型名`** 保存一个人工综合能力分。
分数为 **0–100 整数**，只表达管理员的相对能力评价，不是客观 IQ、评测结果、
成功概率或倍数尺度；不包含速度、价格、可用性。

提供逐项编辑、持久化、跨供应商比较、供应商复制、配置备份及能力与速度性能图。
**评分不影响现有路由。** 未评分模型不会被赋予默认数值。

## 评分状态与模型标识

| 状态 | 表示 | 含义 |
|---|---|---|
| 已评分 | `status: "rated"`，整数分数及时间 | 有已保存评价，0 分也是有效评分 |
| 未评分 | `status: "unrated"`，`score: null`，`updated_at: null` | 查询成功，但没有评分记录 |
| 未知/错误 | 请求失败，界面明确报错 | 无法确定评分，不能冒充未评分 |

数据库只保存已评分记录，清除就是删除该记录，状态由记录是否存在派生。
供应商以 ID 标识，而非名称或 vendor。多个路由引用同一供应商模型时共享评分；
删除或重建路由映射不会删除评分。

评分 API 精确保留模型名的大小写、首尾空格、Unicode、命名空间和斜杠，不做
trim、模糊匹配、别名归并或 Unicode 归一化。名称不可全空白、不可含 NUL，
上限 1024 UTF-8 字节；超限报错而非截断。上游目录到 Nyro 模型名的转换，仍由现有
供应商协议解析边界负责。

除身份外只记录分数和更新时间，不增加备注、多维能力、推理档位、置信度或历史。
普通保存（包括再次确认同一个分数）使用服务端 UTC RFC3339 毫秒时间；并发编辑
采用最后成功写入者生效。

## 管理界面

- **可用模型**页显示分数或明确的“未评分”，支持逐个编辑。
- **模型评分**页（`/model-ratings`）以平铺列表跨供应商管理，可搜索、按供应商/
  评分状态/分数范围筛选，按分数/名称/更新时间排序及分页。
- 列表为成功目录、已知路由目标、已保存评分三者的并集。禁用供应商和已不在目录中
  的评分仍可管理。
- 默认分数降序；无论升序或降序，未评分始终置后。分数范围只包含已评分记录；
  同分按稳定的身份顺序排列。
- 输入必须是整数，不取整、不夹到范围内。空输入不等于 0，也不等于清除；清除使用
  独立确认操作。
- 评分请求失败时显示错误与重试，不能展示虚假的未评分数量。目录请求失败时说明
  列表可能不完整，但不妨碍编辑已保存评分。
- 只有成功目录中未出现的模型才标记“目录缺失”；禁用或加载失败显示“目录未知”。

评分管理页通过 `require_catalog=true` 获取目录，明确报告网络、HTTP、JSON 或
目录结构错误，而不是使用旧逻辑静默返回空目录/静态兜底。其他目录消费者保留
原有行为。保存和清除评分本身不查询上游。

## 管理接口

所有接口沿用现有 Admin API 认证，不暴露到代理侧的公共模型元数据。
桌面 IPC 调用相同的核心服务。

| HTTP | 用途 |
|---|---|
| `GET /api/v1/provider-model-ratings` | 所有已保存评分，包括禁用和目录缺失记录 |
| `GET /api/v1/provider-model-ratings?provider_id=…` | 某供应商的已保存评分 |
| `GET /api/v1/providers/:id/model-rating?model=…` | 某组合的明确已评分/未评分状态 |
| `PUT /api/v1/providers/:id/model-rating?model=…` | 保存 `{ "score": 85 }` 并返回记录 |
| `DELETE /api/v1/providers/:id/model-rating?model=…` | 清除单项，返回 `{ "ok": true }` |

模型名作为查询参数需要 URL 编码，尤其 `/`、`+`、`#` 和空格。
列表、查询、保存沿用 `{ "data": ... }` 响应封装，例如：

```json
{
  "data": {
    "provider_id": "provider-id",
    "upstream_model": "vendor/model-x",
    "status": "unrated",
    "score": null,
    "updated_at": null
  }
}
```

已评分时 `status` 为 `rated`，`score` 是包含 0 在内的整数，时间非空。
PUT 拒绝缺省/null/字符串/小数/超范围评分，以及客户端设置更新时间等多余字段。
有效供应商下重复清除幂等成功。评分错误使用 JSON `{ "error": "..." }`，状态码
为 400 输入错误、404 供应商不存在、501 后端不支持、500 内部失败；存储错误
不能转换成空列表成功。

列表枚举的是**已保存评分**，不是全部未评分模型。未来消费者必须在评分读取成功后
与自己的候选模型集合关联，或查询单项明确状态。目录不可用时，无法统计尚未发现
的未评分模型。

## 生命周期与备份

| 事件 | 评分处理 |
|---|---|
| 目录刷新、临时下架、探测失败 | 保留分数和时间 |
| 禁用供应商、更换地址/账号/渠道 | 保留，由管理员按需重新评价 |
| 删除路由映射 | 保留 |
| 同一供应商下精确同名模型重新出现 | 复用原分 |
| 新名称或别名出现 | 不自动继承 |
| 清除评分 | 只删除指定组合 |
| 删除供应商 | 在事务中删除该供应商全部评分 |
| 复制供应商 | 分数与原时间复制到新 ID，之后各自独立 |
| 导出配置 | 在所属供应商下写入 `model_ratings`，包含禁用/目录缺失记录 |
| 导入新供应商 | 绑定新 ID，保留原评价时刻，格式统一为 UTC 毫秒 |
| 导入时供应商同名已存在 | 供应商及其评分整体跳过，不覆盖当前评分 |

导出是现有版本 2 的兼容字段扩展，无评分字段的旧备份仍可导入。旧版 Nyro 可能
忽略新字段，不能指望它完整保留评分。

导入修改配置前先校验全部评分、标识、时间与重复精确键。某供应商评分恢复失败时
回滚刚创建的供应商，但不全局回滚先前已成功导入的其他供应商；错误反馈说明
部分完成情况。复制评分失败同样报错并回滚新供应商，不返回无评分的“成功副本”。

## 性能图（Performance chart）

**性能**页（`/performance`）为每个已评分的精确供应商模型组合显示一个点：横轴为
综合分（评分本身仍为 0–100），下限按当前可见绘图点的最低分向下取整至 10 的倍数，上限按最高分向上取整至 10 的倍数，且始终限制在 0–100。单个点至少保留 10 分跨度；没有点时使用完整 0–100。例如 53–78 → 50–80，只有 73 → 70–80，只有 100 → 90–100。刻度间隔为 10，搜索和供应商筛选后重新计算坐标范围，但不改变实际评分或点的身份。极值分映射在坐标轴内侧一个标记边距处（半个标记加余量），最左/最右的标记不会压在坐标轴上；点、刻度数值与包络线共用同一套映射。
纵轴为统一主 TPS（正文端到端吞吐 = 正文 Token ÷ 请求耗时），默认 0–100，超过 100 时按 50 TPS 步长扩大上限并保留余量。有效的 0 TPS（保留样本全部为纯推理）作为真实数据点绘制在基线上——0 是数据不是缺失，只有 null（无有效样本）才缺失。不按推理强度评分、筛选或拆点。
只有位于可见右上凸包边界（外围虚线）上的点直接标模型名，同名模型加供应商区分，
边界上的重合组列出该处所有模型；其余点一律不直接标名，悬停、键盘聚焦或移动端轻触时显示。
标签换行避碰但不移动真实点坐标；包络线位置过密而无法容纳的标签会提示隐藏。
悬停、键盘聚焦或移动端轻触可查看评分、TPS 和样本详情浮层，离开或 Esc 关闭；
不再常驻侧栏索引或显示点编号。保留搜索和供应商筛选。

### 右上能力–速度包络线

虚线仅连接**当前可见有效点的右上凸包边界**，不是把所有帕累托点连接起来。
例如 A(40,200)、B(60,120)、C(90,100) 时连接 A–C，跳过凹进去的 B；B 仍作为普通点显示。
搜索、供应商筛选和刷新后重算边界；坐标缩放及显示小数位数不决定模型是否属于边界。

- 样本较少的点也参与，保留样本提醒与虚线图标边框。
- 同分取最快，同速取最强；完全同坐标的全部模型共享边界归属。
- 斜向边界上的共线中间点也算边界模型。位于线下的点，即使不被某个单一模型完全压过，
  也不会作为最外层包络线节点。
- 无点不画线；只剩一个最优位置时不画线段，但对应模型的详情仍显示包络线归属。
- 使用统一灰蓝色虚线，绘制在点和标签下方，不填充、不闭合、不延伸到坐标轴，不拦截鼠标。
  供应商颜色和低样本标记的含义不变，详情及可访问描述明确标识边界模型。
- 只有边界成员直接标名；内部或受支配的点一律不直接标名，边界重合位置的全部模型
  合并为同一个标签；悬停、聚焦或轻触可查看任何非边界点的名称。
- 移除横纵网格线，保留左侧/底部实线坐标轴、短刻度、刻度数值、轴标题和模型名称引线。

虚线只是当前观测值的外边界，不代表性能保证或推荐；线段中间不对应实际测得的模型，
低样本仍可能不稳定。算法使用原始分数/TPS，仅在方向判断中容忍机器浮点误差，不按一位小数
的显示值合并或归类。

`GET /api/v1/model-performance`（可选 `provider_id`）及桌面 `get_model_performance`
返回 `{ as_of, window_start, models }`。每项包含 `rating: ProviderModelRating`、
`mixed`、`status`、可选 `error`、`unclassified_count` 和 `untrusted_count`，不再返回
`profile` 或 `tiers`。`mixed` 提供已选/有效请求数、`average_tps`（主正文 TPS，与别名
`overall_tps` 同值）、`average_gross_tps`/`overall_gross_tps`（同一耗时上的总输出 Token，仅辅助）
和样本起止时间。同一别名对两个名字都出现时必须一致，陈旧不一致的取值会被整批拒绝。

一个分组是「一个已评分前缀 × 一个供应商」。`variants` 列出命中该前缀的不同上游模型名，
每个变体各自取最近五十次已保留调用得到自己的 `mixed`；分组级 `mixed` 将各变体的有效样本
正文 Token、输出 Token 与耗时求和合并（TPS = ΣToken ÷ Σ耗时，绝不做逐请求速率的算术平均）：
计数为求和结果，可以合法地超过 50（每个变体最多 50 条），样本起止时间为合并后的
区间，整组都没有可用 TPS 时平均值为 null。

性能页直接采用**与模型调用统计相同的最近五十次已保留调用及 TPS 计算口径**，不再独立要求
完成状态可信，也不额外限制七天、HTTP 状态、推理强度或元数据版本。`window_start` 为 null，
`as_of` 表示统计获取时间。完成状态未知（例如尚未识别结束格式的 MiniMax 响应）不会使已有
输出 token 和耗时失效；选中的五十条内无法计算 TPS 的请求不由更早记录补位。

后端与模型调用统计、日志页共享同一单请求 TPS 契约：正文 Token =
`max(output_tokens − max(reasoning_tokens, 0), 0)`；耗时取 `latency_upstream_ms`，仅当其为
null/undefined 时回退 `latency_total_ms`（0、负数或非有限值一律无效，绝不扣除首字延迟）；
`output ≤ 0` 视为无用量，而输出 > 0 的纯推理请求是正文为 0 的有效样本。主 TPS =
Σ正文 ÷ Σ有效请求耗时；总输出 TPS（辅助）= Σ输出 ÷ 同一耗时，主指标绝不回退为总输出 TPS。
未提供或历史未记录推理明细时，正文按已报告输出计算，不能保证精确扣除推理 Token。
点保留完整数值精度；展示用一位小数，0 显示 `0.0`，只有 null 显示 `–`。
每个点绘制该供应商的厂商图标，不再使用圆点，且只画在自己的标记框内：图标文件自带其它尺寸时
统一归一化撑满该框，不再盖到图上。身份取供应商的规范预设/厂商标识（即管理面已有的
`provider_icon`，如 `ark-coding` → doubao），其次按名称与主机解析，最后回退首字母；
协议不参与解析——`openai-compatible`、`openai-responses` 描述的是请求格式，用它匹配会把
所有中转商都标成 OpenAI。`custom` 预设没有可用的矢量标记，这类供应商保留名称与主机所识别的身份。
少于三个有效样本时该标记改为虚线边框并降低不透明度；坐标保留原始精度，显示保留一位小数；缺少数据与读取失败明确
区分，不主动向上游测速。生命周期元数据及历史恢复仍可用于诊断，但不再阻止样本进入性能图。
为兼容旧客户端，`unclassified_count` 和 `untrusted_count` 字段保留并返回 0，界面不再将其
展示成无效调用数量。

### 取消分档后的兼容策略

保留物理 `effort` 列及旧非 `common` 记录，不做破坏性迁移。仅 `common` 记录作为
单一综合评分展示、复制和导出。仅有旧分档评分的模型视为未评分，需管理员手动给出
综合分，不自动平均或选择某个档位兜底。备份未带 `effort` 或显式为 `common` 均接受；
非 `common` 评分条目拒绝导入，不静默转换。

## 架构与验证

`ProviderModelRatingStore` 独立于路由快照、后端权重和第三方模型能力目录。
SQLite、PostgreSQL、MySQL 均持久化评分；YAML/MemoryStorage 保持现有只读/
不支持边界，不把重启即丢失的操作伪装成持久化保存。

单独修改评分不改变 `config_epoch`，不重载路由缓存或刷新配额。两处 UI 共享
经过校验的评分查询，修改后失效缓存，评分刷新与目录刷新独立。
数据库结构与临时库生成参考 SQL 的方法见[数据库文档](../database/schema.md)。

针对性验证命令：

```bash
cargo test -p nyro-core --test provider_model_ratings
cargo test -p nyro-core --test storage_provider_model_ratings
cargo test -p nyro-server --no-default-features rating_
cargo check -p nyro-core
cargo check -p nyro-desktop
```

WebUI 在 `webui/` 下执行 lint/build。轻量 UI helper 测试不需要额外测试框架：

```bash
# 在 webui/ 下执行
test_dir=$(mktemp -d /tmp/nyro-rating-tests.XXXXXX)
./node_modules/.bin/tsc src/lib/model-ratings.test.ts src/lib/model-performance.test.ts --outDir "$test_dir" \
  --module commonjs --moduleResolution node --target es2020 \
  --esModuleInterop --skipLibCheck
node --test "$test_dir/model-ratings.test.js" "$test_dir/model-performance.test.js"
```

存储测试通过显式的临时库变量开启
PostgreSQL/MySQL 实测；跳过不等于该后端验证通过。`tests/webui/` 的 Node/CDP
冒烟测试只使用隔离的临时服务，验证真实浏览器操作，不连接现有部署。
