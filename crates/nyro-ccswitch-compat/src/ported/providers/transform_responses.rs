...
pub(crate) fn build_anthropic_usage_from_responses(usage: Option<&Value>) -> Value {
    ...
    if u.as_object().map(|obj| obj.is_empty()).unwrap_or(false) {
        tracing::warn!("[Responses] Empty usage object received, using defaults");
        return json!({
            "input_tokens": 0,
            "output_tokens": 0
        });
    ...
    let output = u.get("output_tokens")
        ...
        .or_else(|| {
            ...
            if completion_tokens.is_some() {
                tracing::debug!("[Responses] Using OpenAI field name fallback 'completion_tokens' for output_tokens");
            }
            completion_tokens
        })
        .unwrap_or(0);
    ...
    if (input == 0 && output > 0) || (input > 0 && output == 0) {
        tracing::debug!("[Responses] Partial usage object: {:?}", u);
    }
