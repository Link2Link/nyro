            ...
            match chunk {
                Ok(bytes) => {
                    ...
                    while let Some(block) = take_sse_block(&mut buffer) {
                        ...
                        tracing::debug!("[Claude/Responses] <<< SSE event: {event_name}");
                        ...
                        match event_name {
                            ...
                            "response.created" => {
                                ...
                                tracing::debug!("[Claude/Responses] >>> Anthropic SSE: message_start");
                            ...
                            "response.completed" | "response.incomplete" => {
                                ...
                                tracing::debug!("[Claude/Responses] >>> Anthropic SSE: message_delta");
                                yield Ok(Bytes::from(sse));
                                ...
                                let stop_sse = format!("event: message_stop\ndata: {}\n\n",
                                    serde_json::to_string(&stop_event).unwrap_or_default());
                                tracing::debug!("[Claude/Responses] >>> Anthropic SSE: message_stop");
                                yield Ok(Bytes::from(stop_sse));
                                terminated = true;
                            }
