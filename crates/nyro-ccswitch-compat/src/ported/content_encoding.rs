        ...
        "deflate" => {
            ...
            match read_with_output_limit(zlib, max_output_bytes) {
                ...
                Err(zlib_err) => {
                    ...
                    tracing::debug!("deflate 按 zlib 解压失败（{zlib_err}），回退 raw deflate");
...
pub fn decompress_body_with_limit(
    ...
    if !codings.iter().all(|c| is_single_supported(c)) {
        tracing::warn!("不支持的 content-encoding: {content_encoding}，跳过解压");
        return Ok(None);
    }
...
pub fn get_content_encoding(headers: &[Header]) -> Option<String> {
...
mod tests {
    ...
    fn encoding_header(value: &str) -> Header {
        Header::new("content-encoding", Bytes::copy_from_slice(value.as_bytes()))
    }
    ...
    fn get_content_encoding_combines_repeated_headers() {
        ...
        let headers = vec![encoding_header("gzip"), encoding_header("zstd")];
        assert_eq!(
    ...
    fn get_content_encoding_ignores_identity_only() {
        let headers = vec![encoding_header("identity")];
